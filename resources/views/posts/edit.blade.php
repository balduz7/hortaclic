@extends('layouts.app')
@section("content")
<form id="validacion-posts-edit" method="post" action="{{ route('posts.update', $post) }}" enctype="multipart/form-data" class="bg-white p-8 rounded shadow-md">
    @csrf
    @method('PUT')
    
    <label for="body">{{__('Description')}}</label><br>
    <textarea id="body" name="body" rows="4" cols="50" placeholder="Ingresa tu texto aquí...">{{$post->body}}</textarea>
    <div id="body-error" class="error-message" style="display:none;color:red;"></div>
    <br>

    <label for="latitude">{{__('Latitude')}}</label><br>
    <input type="number" id="latitude" name="latitude" step="0.000001" value="{{$post->latitude}}">
    <div id="latitude-error" class="error-message" style="display:none;color:red;"></div>
    <br>

    <label for="longitude">{{__('Longitude')}}</label><br>
    <input type="number" id="longitude" name="longitude" step="0.000001" value="{{$post->longitude}}">
    ...
    <select id="visibility_id" name="visibility_id" class="form-input w-full border rounded py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
        ...
        <option value="3" {{$post->visibility_id == 3 ? 'selected' : ''}}>Private</option>
    </select>
    ...
    <div class="flex justify-between">
        <button type="submit" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline">{{__('Editar')}}</button>
    </div>
</form>
